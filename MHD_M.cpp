...
std::vector<double> getXi_VL(std::vector<double> r){
    std::vector<double> xi(8);
...
std::tuple< std::vector<double>,std::vector<double>  > calc_ubar(const std::vector<double>& u_minus,const std::vector<double>& u_0, const std::vector<double>& u_plus,double w){
    ...
    std::vector<double> xi= getXi_VL(r);
...
int main(void){
    ...
    do {
        ...
        //Flux is the standard HLLC flux, but uL and uR are replaced with the corresponding ubar_plus values from SLIC

        for(size_t i=2; i<num_xcells+2; i++){
            for(size_t j=0;j<8;j++){
                uPlus1[i][j]=u[i][j] - (dt/dx) * (get_HLLC_flux(ubar_R_plusx[i],ubar_L_plusx[i+1],gamma)[j]-get_HLLC_flux(ubar_R_plusx[i-1],ubar_L_plusx[i],gamma)[j]);
